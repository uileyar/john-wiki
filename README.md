...
CLAUDE.md   schema、命名规范、slash 命令和工作流约定
```

Obsidian 负责浏览、搜索和图谱视图；Claude Code 负责摄入、查询、体检和图谱生成。正文和 prompt 默认中文，frontmatter 字段名保留英文以兼容 Dataview 与上游工具。
...
1. 把不可变源放到 `raw/` 下对应目录。
2. 在 Claude Code 里运行 `/wiki-health` 做预检。
