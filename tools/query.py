...
def query(question: str, save_path: str | None = None):
    ...
    if not index_content:
        print("wiki 里还没东西，先用 `/wiki-ingest` 喂点源。")
        sys.exit(1)
    if not any((WIKI_DIR / "sources").glob("*.md")):
        print("wiki 里还没东西，先用 `/wiki-ingest` 喂点源。")
        sys.exit(0)
    ...
    if not relevant_pages or len(relevant_pages) <= 1:
        ...
        prompt = f"给定这个 wiki/index.md：\n\n{index_content}\n\n哪些页面最适合回答这个问题：\"{question}\"\n\n只返回一个 JSON 数组，内容是 index 中出现的相对文件路径，例如 [\"sources/foo.md\", \"concepts/Bar.md\"]。最多 10 个页面。"
    ...
    prompt = f"""你正在查询一个以中文为主的 LLM Wiki。请只基于下面的 wiki 页面综合回答用户问题。引用必须使用 [[页面名]] wikilink 语法。
...
请输出结构清晰的中文 markdown 回答，所有关键依据都用 [[wikilink]] 行内引用。末尾添加 `## 引用源`，列出用到的页面。
"""
...
"""
        ...
        # Update index
        index_content = read_file(INDEX_FILE)
        entry = f"- [{question[:60]}]({save_path}) —— 回答：{question[:40]}"
        if "## 综合（Syntheses）" in index_content:
            index_content = index_content.replace("## 综合（Syntheses）\n", f"## 综合（Syntheses）\n{entry}\n")
        elif "## Syntheses" in index_content:
