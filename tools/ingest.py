...
def update_index(new_entry: str, section: str = "源"):
...
def ingest(source_path: str, auto_convert: bool = True):
    ...
    if source.suffix.lower() != ".md":
        ...
        if source.suffix.lower() not in CONVERTIBLE_EXTENSIONS:
            print(f"  warning: unsupported format: {source.suffix} - skipping {source.name}")
    ...
    prompt = f"""你正在维护一个以中文为主的 LLM Wiki。请处理下面的源文档，并把它的知识整合进 wiki。

Schema 与工作流约定：
{schema}

当前 wiki 状态（目录、综述和最近源页面）：
{wiki_context if wiki_context else "(wiki 目前为空，这是第一个源)"}

待摄入的新源（文件：{source.relative_to(REPO_ROOT) if source.is_relative_to(REPO_ROOT) else source.name}）：
=== SOURCE START ===
{source_content}
=== SOURCE END ===
...
请只返回一个合法 JSON 对象，不要 markdown fences，不要 JSON 外的解释文字。除专有名词和原文引用外，所有页面正文、摘要、log、index 摘要默认用中文：
{{
  "title": "这个源的中文可读标题",
  "slug": "kebab-case-slug-for-filename",
  "source_page": "wiki/sources/<slug>.md 的完整 markdown 内容。使用 CLAUDE.md 中的中文源页面模板。重要：把关键人物、产品、概念、项目积极写成 [[Wikilinks]]；已知术语漏掉 [[ ]] 视为失败。",
  "index_entry": "- [标题](sources/slug.md) —— 一行中文摘要",
  "overview_update": "wiki/overview.md 的完整更新内容；如果无需更新则为 null",
  "entity_pages": [
    {{"path": "entities/EntityName.md", "content": "完整 markdown 内容，正文中文"}}
  ],
  "concept_pages": [
    {{"path": "concepts/ConceptName.md", "content": "完整 markdown 内容，正文中文"}}
  ],
  "contradictions": ["用中文描述与现有 wiki 的矛盾；如果没有则为空数组"],
...
"""
    ...
    # Update index
    update_index(data["index_entry"], section="源")
    ...
    if validation["broken_links"]:
        print(f"  warning: broken links: {len(validation['broken_links'])}")
    ...
    if validation["unindexed"]:
        print(f"  warning: not in index.md: {len(validation['unindexed'])}")
    ...
    for arg in args:
        ...
        if p.is_file():
            ...
            if ext in ALL_SUPPORTED_EXTENSIONS:
                ...
                print(f"  warning: skipping unsupported format: {p.name} ({ext})")
        elif p.is_dir():
            for f in p.rglob("*"):
